import SafeQuestionTable from '@/pages/Question/QuestionList/components/SafeQuestionTable';
import { getSubmitSummary } from '@/services/questionSubmit/api';
...
const QuestionSet: React.FC = () => {
  ...
  const [summary, setSummary] = useState<QuestionSubmit.SubmitSummary | null>(null);

  const getTotalByName = (name: string, data: QuestionSubmit.SubmitSummary) => {
    switch (name) {
      case '简单':
        return data.easyTotal;
      case '中等':
        return data.mediumTotal;
      case '困难':
        return data.hardTotal;
      case '未通过':
        return data.total;
    }
  };
      ...
      getSubmitSummary()
        .then((res) => {
          if (res.code === 200) {
            ...
            let totalPass = Number(data.easyPass) + Number(data.mediumPass) + Number(data.hardPass);
              ...
              series: [
                {
                  ...
                  data: [
                    ...
                    {
                      value: data.total - data.easyPass - data.mediumPass - data.hardPass,
                      name: '未通过',
                    },
                  ],
                },
