...
const SecurityView: React.FC = () => {
  ...
  const [showPhone, setShowPhone] = useState(() => {
    ...
    return `已绑定手机：${currentUser.phone.substring(0, 3)}****${currentUser.phone.substring(
      7,
      11,
    )}`;
  });
  ...
  const [showEmail, setShowEmail] = useState(() => {
    ...
    return `已绑定邮箱：${currentUser.email.substring(0, 3)}***${currentUser.email.substring(
      index - 3,
    )}`;
  });
  ...
  useEffect(() => {
    if (!currentUser?.phone) {
      ...
      setShowPhone(
        `已绑定手机：${currentUser.phone.substring(0, 3)}****${currentUser.phone.substring(7, 11)}`,
      );
    }
    if (!currentUser?.email) {
      setShowEmail('暂未绑定邮箱');
      ...
      if (index <= 6) {
        ...
        setShowEmail(
          `已绑定邮箱：${currentUser.email.substring(0, 3)}***${currentUser.email.substring(
            index - 3,
          )}`,
        );
      }
    ...
    <>
      {currentUser && (
        <>
          ...
          />
          <PasswordModal visible={passwordModal} onCancel={() => setPasswordModal(false)} />
        </>
      )}
    </>
  );
