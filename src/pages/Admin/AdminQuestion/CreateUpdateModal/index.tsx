...
const CreateUpdateModal: React.FC<CreateModalProps> = ({
  visible,
  targetId,
  onCancel,
  reloadData,
      ...
      getQuestionVoById(targetId).then((res) => {
        if (res.code === 200) {
          ...
          formRef.current?.setFieldValue(
            'judgeCase',
            questionVo.judgeCase.map((item, index) => {
              return {
                id: ids[index],
                input: item.input,
                output: item.output,
  ...
  const createUpdateQuestion = () => {
    ...
    if (targetId === -1) {
      ...
      updateQuestion({ id: targetId, ...params }).then((res) => {
        if (res.code === 200) {
        ...
        >
          ...
          <ProForm.Item label="内存限制" name="memoryLimit">
            <InputNumber
              addonAfter="MB"
              value={memoryLimit}
              onChange={(value) => setMemoryLimit(value)}
            />
          </ProForm.Item>
          ...
          <ProForm.Item label="堆栈限制" name="stackLimit">
            <InputNumber
              addonAfter="MB"
              value={stackLimit}
              onChange={(value) => setStackLimit(value)}
            />
          </ProForm.Item>
        </ProForm>
      </div>
